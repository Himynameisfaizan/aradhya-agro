...
<title>Contact Us | Aradhya Agro Industries</title>
<meta name="description" content="Contact Aradhya Agro Industries for retail orders, bulk enquiries, distributorship and export.">
...
<style>
  .contact-card{ background:var(--maroon); color:#fff; border-radius:22px; padding:2.6rem; box-shadow:0 20px 40px rgba(122,30,43,.3); }
  .contact-card h3{ font-family:'Yatra One',cursive; color:var(--gold-bright); }
  .contact-line{ display:flex; gap:.8rem; align-items:flex-start; margin-bottom:1.1rem; }
  .contact-line i{ color:var(--gold-bright); font-size:1.15rem; margin-top:.15rem; }
  .contact-line span{ font-size:.94rem; color:#F3E6C4; }
  .hero-tagline{ font-family:'Yatra One',cursive; color:var(--maroon); font-size:1.25rem; margin-top:1.6rem; border-left:3px solid var(--gold); padding-left:.9rem; }
  .form-control{ background:var(--cream); border:1px solid rgba(122,30,43,.15); border-radius:10px; padding:.7rem .9rem; }
  .form-control:focus{ box-shadow:0 0 0 .2rem rgba(201,138,2,.25); border-color:var(--gold); }
  .btn-submit{ background:var(--green); color:#fff; font-weight:700; border-radius:10px; padding:.7rem 1.4rem; border:none; }
  .btn-submit:hover{ background:#163a28; color:#fff; }
  .map-embed{ border-radius:16px; overflow:hidden; box-shadow:0 10px 30px rgba(42,33,20,.1); }
</style>
...
<section class="page-hero">
  <div class="container">
    ...
    <p>Retail orders, bulk enquiries, distributor applications, or export quotes — our team typically replies within 24 hours.</p>
  </div>
</section>
...
<section class="section-pad">
  <div class="container">
    <div class="row g-4 align-items-stretch">
      <!-- Contact Info Card (Same as yours) -->
      <div class="col-lg-5">
        <div class="contact-card h-100">
          <h3 class="mb-4">Aradhya Agro Industries Pvt. Ltd.</h3>
          <div class="contact-line"><i class="bi bi-geo-alt-fill"></i><span>House No. 05/97, Ward No. 5, Kabir Nagar, Nagar Panchayat Paniyara, Post: Paniyara, District: Maharajganj, Uttar Pradesh – 273310, India</span></div>
          <div class="contact-line"><i class="bi bi-telephone-fill"></i><span>+91 9278480163 (WhatsApp) &nbsp;|&nbsp; +91 9936698184</span></div>
          <div class="contact-line"><i class="bi bi-envelope-fill"></i><span>aradhyaagroindustriespvtltd@gmail.com</span></div>
          <div class="contact-line"><i class="bi bi-patch-check-fill"></i><span>FSSAI: 12725033000119 &nbsp;|&nbsp; DPIIT: DIPP256135</span></div>
          <p class="mt-4 mb-0 hero-tagline" style="border-color:var(--gold-bright); color:#fff;">Purity • Trust • Health • Global</p>
        </div>
      </div>
      ...
      <div class="col-lg-7">
          <form id="contactForm" class="row g-3 bg-white p-4 p-md-5 rounded-4 shadow-sm h-100">
            ...
            <div class="col-12">
              ...
              <textarea name="message" class="form-control" rows="3" placeholder="Tell us about your requirements..."></textarea>
            </div>
            <div class="col-12">
...
<!-- Google Map Section (Same as yours) -->
<section class="section-pad" style="background:var(--cream-deep); padding-top:0;">
  <div class="container">
    <div class="map-embed">
      <iframe src="https://www.google.com/maps?q=Paniyara,Maharajganj,Uttar+Pradesh&output=embed" width="100%" height="360" style="border:0;" allowfullscreen="" loading="lazy" title="Aradhya Agro Industries location"></iframe>
    </div>
  </div>
</section>
...
document.getElementById('contactForm').addEventListener('submit', async function(e) {
    e.preventDefault(); // Default form submit rokega
    
    let btn = document.getElementById('submitBtn');
    let responseDiv = document.getElementById('formResponse');
    let formData = new FormData(this);
    
    // Button state update (Loading...)
    ...
    try {
        ...
        // Output Message dikhane ke liye
        responseDiv.style.display = 'block';
        if (result.status === 'success') {
            responseDiv.className = 'alert alert-success';
            responseDiv.innerHTML = result.message;
            this.reset(); // Success hone pe inputs clean honge
        } else {
